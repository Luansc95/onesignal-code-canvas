...
3. **Leads (marco crítico)** — backup em JSON dos dados locais; reescrever a persistência de `leadService` para o banco preservando validação, UTM e scoring; fluxo formulário → lead → score → atividade → evento de analytics → notificação. `/admin/leads` com filtros/busca/paginação server-side, status, prioridade, follow-up (`next_follow_up_at`, `assigned_to`, notas internas), histórico, arquivamento e realtime. **Critério de aceite: um Lead criado pelo site público aparece em `/admin/leads` com score, origem, UTM, atividade e timestamp reais.** Só depois disso o BI avança.
...
7. **Analytics real** — remover `generateRealisticHistoricalEvents()`, seeds e fallbacks; `visitor_id`/`session_id`/`event_id` anônimos (sem PII); persistir todos os eventos listados com UTM, referrer, landing page e metadata; cliques de WhatsApp e Instagram rastreados.
8. **BI, funil e origem** — dashboard alimentado por funções de agregação SQL por período (hoje, 7d, 30d, mês, personalizado), distinguindo visitantes únicos, sessões e page views; funil visitantes → interações → diagnóstico/formulário → lead → contato → negociação → conversão; relatório de origem first-touch e last-touch. Taxas só quando houver dados.
...
10. **Limpeza e estados vazios** — após backup e migração confirmados, remover seeds e números fallback; mensagens como "Você ainda não possui Leads." e "Não há dados suficientes para este relatório."
11. **Validação** — build + TypeScript, testes de autorização por papel (incluindo anônimo bloqueado em leads/contatos/diagnósticos/analytics) e teste do fluxo ponta a ponta, projeto rascunho invisível no site, logout e refresh de sessão.
